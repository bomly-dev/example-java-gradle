...
This repository is a [Bomly](https://bomly.dev) example project for **Java** with **Gradle**.
...
| `settings.gradle` | Dependency manifest |
...
```bash
bomly scan --url https://github.com/bomly-dev/example-java-gradle
```
...
```bash
bomly scan --url https://github.com/bomly-dev/example-java-gradle --enrich --audit
```

### 3. Trace why a dependency exists

Understand every path that pulls `org.springframework:spring-web` into your graph:

```bash
bomly explain org.springframework:spring-web --url https://github.com/bomly-dev/example-java-gradle
```
